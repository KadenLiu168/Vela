...
- **THEN** 三个页面的 `page-heading h2`（Detail）/`page-heading h1`（Dashboard）必须使用相同的 `font-size`、`line-height`、`letter-spacing` 与 `font-weight`
- **AND** 三个页面的 `page-heading p`（eyebrow）必须使用相同的 `font-size`、`text-transform` 与 `letter-spacing`
