...
- **THEN** 三个 detail 页面的 `.holdings-section` 章节标题（Walk-forward Detail 的 `h2` 与 Signal / Backtest Detail 的 `h3`）必须使用相同的 `font-size`（`var(--text-subheading)`）、`line-height`（`var(--leading-subheading)`）、`letter-spacing`（`var(--tracking-subheading)`）与 `font-weight`（`var(--font-weight-medium)`）
