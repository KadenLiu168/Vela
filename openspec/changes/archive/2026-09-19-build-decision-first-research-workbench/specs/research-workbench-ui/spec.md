# research-workbench-ui Specification Delta
...
- **THEN** the control does not use the acid-lime primary fill
- **AND** the Dashboard's existing Bootstrap action remains the only acid-lime filled button in the view
...
Every decision layer SHALL use existing design tokens, SHALL remain keyboard operable with programmatically associated labels, and SHALL NOT cause page-level horizontal overflow at 1440x1000 or 390x844. A dense holdings table MAY use a labeled local scroll region.
...
- **THEN** no new button variant, no acid-lime filled button beyond the view's primary CTA, and no non-token line-height is introduced
