...
The trend chart SHALL render the backward-adjusted price series as a line and SHALL surface per-point readout on hover.
...
#### Scenario: Hover surfaces date and price
- **WHEN** the user hovers over the chart
- **THEN** the chart indicates the nearest data point
- **AND** a readout displays that point's `trade_date` and `price`
